...
function setupEventListeners() {
  ...
  document.querySelectorAll(".tab").forEach((tab) => {
    tab.addEventListener("click", function () {
      ...
      document
        .getElementById(this.getAttribute("data-tab") + "-content")
        .classList.add("active");
      addMemberBtn.style.display =
        this.getAttribute("data-tab") === "completed" ? "none" : "flex";
    });
  });
  ...
  try {
    ...
    if (response.ok) {
      ...
      participants.training = allUsers.filter(
        (user) => user.baptismStatus === "training",
      );
      participants.completed = allUsers.filter(
        (user) => user.baptismStatus === "completed",
      );
      updateParticipantsList();
      updateCounts();
    } else {
...
async function searchUsers(name) {
  try {
    ...
    if (!token) {
      searchResults.innerHTML =
        '<div class="search-result-item">Authentication error</div>';
      return;
    }

    const response = await fetch(
      `${BASE_API_URL}/user/search?name=${encodeURIComponent(name)}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
async function addParticipant(user) {
  try {
    ...
    if (!regRes.ok) {
      alert(
        "Failed to register candidate for sacrament: " + (await regRes.text()),
      );
      return;
    }
...
async function batchCompleteTraining() {
  ...
  if (!confirm("Mark ALL candidates as completed? This cannot be undone."))
    return;
